...
public class TestInput extends Scanner implements FocusListener {
	...
	public void operators() { // 7 procedure mccabe=1 for start of procedure
		...
		if (i > 0) {          // mccabe=4 for "else" stmt
	    	i = 2;
	    }
	    else                  // mccabe=5 for "else" stmt 
	    if (!false) {         // mccabe=6 for "else" stmt
	    	i = 3;
	    }
		assert (i > 0);       
	}
	 ...
	 * @param args
	 */
	public static void main(String[] args) { // 28 procedure mccabe=1 for start of procedure
		Integer j = 0;
		boolean found;
